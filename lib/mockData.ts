import { Shop, Service, Formula, Reservation, Lead, VehicleSizeSupplement, AddOn, ShopVehicleSize, ShopServiceCategory, DEFAULT_VEHICLE_SIZES, DEFAULT_SERVICE_CATEGORIES } from '../types/database';
...
export const mockShop: Shop = {
    ...
    supportedVehicleSizes: ['S', 'M', 'L', 'XL'], // Deprecated but kept for compatibility
    schedule: {
...
// Mock data for new customizable features
export const mockVehicleSizes: ShopVehicleSize[] = DEFAULT_VEHICLE_SIZES.map((size, index) => ({
    ...size,
    id: `mock-size-${index + 1}`,
    shopId: mockShop.id,
    createdAt: new Date().toISOString()
}));

export const mockServiceCategories: ShopServiceCategory[] = DEFAULT_SERVICE_CATEGORIES.map((category, index) => ({
    ...category,
...
// Mock services using new categoryId structure
export const mockServices: (Service & { formulas: Formula[], supplements: VehicleSizeSupplement[], specificAddOns: AddOn[] })[] = [
    {
        id: 'mock-service-1',
        shopId: mockShop.id,
        name: 'Nettoyage Intérieur Essentiel',
        description: 'Aspiration complète, dépoussiérage des plastiques, nettoyage des vitres. Idéal pour un entretien régulier.',
        categoryId: mockServiceCategories[0].id, // Interior Services
        basePrice: 80,
        baseDuration: 120,
        status: 'active',
        imageUrl: 'https://images.unsplash.com/photo-1551431804-83e78328639e?q=80&w=2070&auto=format&fit=crop',
        createdAt: new Date().toISOString(),
        formulas: [
        ...
        supplements: [
            { id: 's1', serviceId: 'mock-service-1', vehicleSizeId: mockVehicleSizes[1].id, additionalPrice: 15, additionalDuration: 30, createdAt: new Date().toISOString() },
            { id: 's2', serviceId: 'mock-service-1', vehicleSizeId: mockVehicleSizes[2].id, additionalPrice: 30, additionalDuration: 45, createdAt: new Date().toISOString() },
        ],
        specificAddOns: [
            { id: 'a1', serviceId: 'mock-service-1', shopId: mockShop.id, name: 'Extra: Poils de chien', price: 20, duration: 30, createdAt: new Date().toISOString() }
        ]
    },
    {
        id: 'mock-service-2',
        ...
        description: 'Prélavage, lavage manuel, cire de protection et nettoyage des jantes.',
        categoryId: mockServiceCategories[1].id, // Exterior Services
        basePrice: 90,
        baseDuration: 150,
        status: 'active',
        imageUrl: 'https://images.unsplash.com/photo-1605152273339-a8a1c83c273a?q=80&w=1974&auto=format&fit=crop',
        createdAt: new Date().toISOString(),
        formulas: [
...
export const mockReservations: Reservation[] = [
    {
        id: 'mock-res-1', shopId: mockShop.id, date: futureDate.toISOString().split('T')[0], startTime: '10:00', duration: 120, price: 80, clientName: 'Jean Dupont', status: 'upcoming', paymentStatus: 'paid',
